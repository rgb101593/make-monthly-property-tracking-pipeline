...
Seven properties. Ten orchestration scenarios. Runs monthly, unattended.
...
PM export (email or manual upload)
  ...
  [2] Property tracker       - discover structure, decide target month, patch model
        ↓
  [3] Excel operating model  - 16 sheets, actuals vs. underwriting, waterfall logic
        ↓
...
| Orchestration | Make.com (10 scenarios, ~40 modules each) | Structure discovery, decisioning, patching, publication |
...
- [Sparse-row width inference](case-studies/02-sparse-row-mapping-defect.md) - a fix that introduced a worse bug: inferring array width from the first returned row right-aligned the mapping and corrupted 1,220 cells. Includes the detection and repair path.
